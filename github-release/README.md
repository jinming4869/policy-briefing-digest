...
policy-briefing-digest/
  ...
  configs/
    feishu_targets.yaml             # 飞书文档目标配置模板（填写后重命名为不带 template 后缀的文件）
  content/
...
| 工具 | 用途 | 安装 |
|------|------|------|
| 飞书 CLI | 文档读写 | `npm i -g lark-cli`（需组织内部 npm registry） |
| Microsoft Word | .doc 转 .docx | Windows 预装或 Office 安装 |

### 2. 配置飞书（可选，跳过则仅输出可粘贴文本）

1. 将 `configs/feishu_targets.yaml` 中的 `url` 替换为你的周报飞书 wiki 地址
2. 运行 `lark-cli doctor` 确认认证通过
3. 确认 `tools/feishu_write.ps1` 可以读取目标文档
...
| 编码降级 | 始终用 UTF-8 文件和 `--content @relative-file` 传中文内容 |
...
编辑 `configs/feishu_targets.yaml`，填入你的周报 wiki URL 和各栏目在文档中的标题。

**第四步：验证链路**

手动跑一次写入流程，确认可以读取目标文档并定位栏目标题：

```powershell
powershell -NoProfile -File tools/feishu_write.ps1 -Doc "你的wiki-token" -HeadingId "h4-block-id"
```
